describe("Basic user flow for Website", () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let jsonText = await innerText.jsonValue();
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    await page.reload();
    let shadowRoot, button, buttonText, buttonJson;
    let prodItems = await page.$$("product-item");
    for (let i = 0; i < prodItems.length; i++) {
      shadowRoot = await prodItems[i].getProperty("shadowRoot");
      button = await shadowRoot.$("button");
      buttonText = await button.getProperty("innerText");
      buttonJson = await buttonText.jsonValue();
      expect(buttonJson).toBe("Remove from Cart");
    }
    let cart = await page.$("#cart-count");
    let cartText = await cart.getProperty("innerText");
    let cartJson = await cartText.jsonValue();
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      buttonText = await button.getProperty("innerText");
      buttonJson = await buttonText.jsonValue();
      expect(buttonJson).toEqual("Add to Cart");
    }
    let cart = await page.$("#cart-count");
    let cartText = await cart.getProperty("innerText");
    let cartJson = await cartText.jsonValue();
